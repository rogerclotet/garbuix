...
// the scheduler an hour before rollover, past days are filled by the backfill
// script, and both call ensureDailyPuzzleSnapshot directly. Today is the only
// date a request may still need to bootstrap (a cold start, or a missed
// pre-generation), and generating it discloses nothing a player can't see.
export function triggerDailyPuzzleGeneration(
	dateKey = getTodayDateKey(),
): void {
	if (dateKey !== getTodayDateKey()) {
		console.warn(
			`[puzzle-scheduler] Refusing on-demand generation for ${dateKey}: only today may be generated on demand`,
		);
		return;
	}
